...
\begin{abstract}
We measure the fractal dimension of neural network training trajectories in function space. Using the Grassberger-Procaccia correlation dimension $D_2$, we find that a convolutional network trained on CIFAR-10 with full-batch gradient descent produces a strange attractor with $D_2 = 3.67 \pm 0.08$ at 30\% of the Edge of Stability threshold. Persistent homology confirms fractal rather than smooth manifold topology. Cross-architecture controls reveal that data complexity is the necessary ingredient for multi-dimensional chaos, while architecture modulates the learning-rate threshold at which it appears: a multilayer perceptron on the same data reaches $D_2 = 4.37 \pm 0.05$ near the Edge of Stability, whereas either architecture on structureless synthetic data remains at $D_2 \approx 0.9$. A continuous label-noise sweep confirms that data structure controls attractor complexity, but reveals a regime dependence: at moderate learning rates, degrading label structure smoothly reduces $D_2$ from $3.6$ to $1.7$; near the Edge of Stability, the rougher landscape created by random labels slightly increases $D_2$. Lyapunov exponents peak at intermediate learning rates and become negative at high learning rates as basin convergence dominates. Peak chaos and peak geometric complexity dissociate across all architectures tested---the attractor is most complex in the transition zone between chaotic and basin-convergent regimes, consistent with the KAM transition. These results provide a geometric characterization of training dynamics consistent with the Newhouse-Ruelle-Takens route to chaos.
\end{abstract}
...
\textit{Data complexity is the engine; architecture is the throttle.}---The cross-architecture controls (Table~\ref{tab:conditions}, Fig.~\ref{fig:cross}) reveal a clean hierarchy. A CNN on structureless synthetic data produces $D_2 < 1.0$ at all learning rates---architecture alone is insufficient. A parameter-matched MLP (269K) on CIFAR-10 reaches $D_2 = 4.37 \pm 0.05$ at 90\% of EoS; a smaller MLP (156K) reaches $D_2 = 4.61 \pm 0.10$. Both sit near the Eckmann--Ruelle ceiling for our trajectory length, but the width inversion is significant at $2.2\sigma$: the wider network's additional capacity raises the effective stability margin, delaying bifurcation onset and slightly suppressing peak $D_2$ at a given learning-rate fraction. Width modulates the pace of the transition to chaos, not its destination. Data complexity is both necessary (CNN + synthetic $\to$ $D_2 \approx 1$) and sufficient (MLP + CIFAR-10 $\to$ $D_2 > 4$).
...
\textit{Data complexity as a continuous control parameter.}---To move beyond the binary CIFAR/synthetic comparison, we sweep label-noise fraction $p$ from 0 (clean CIFAR-10) to 1 (uniformly random labels) at each architecture's peak-$D_2$ learning rate (Fig.~\ref{fig:noise}). For the CNN at 30\% of EoS, $D_2$ falls smoothly from $3.64 \pm 0.11$ to $1.68 \pm 0.54$---randomizing labels degrades data structure and destroys the attractor modes. However, for the MLP at 90\% of EoS, $D_2$ rises modestly from $4.36 \pm 0.05$ to $4.77 \pm 0.08$. This divergence reflects a regime dependence: at moderate learning rates, structured data creates the oscillatory modes that the attractor fills, and noise removes them; near the Edge of Stability, random labels roughen the loss surface, creating additional competing minima that sustain higher-dimensional dynamics even without label structure. In both regimes, changing label content continuously changes the attractor---data complexity acts as a genuine control parameter, not a binary switch.

\begin{figure}[t]
\includegraphics[width=\columnwidth]{figures/revision1/label_noise_d2.pdf}
\caption{\label{fig:noise}%
$D_2$ versus label-noise fraction $p$ at each architecture's peak learning rate. CNN at 30\% EoS (red circles): $D_2$ falls as label structure is degraded. MLP~269K at 90\% EoS (blue squares): $D_2$ rises modestly as random labels roughen the loss landscape. Three seeds; error bars: standard deviation.}
\end{figure}

\textit{Peak chaos and peak complexity dissociate.}---For the CNN on CIFAR-10, peak $\lambda$ occurs at 15\% of EoS while peak $D_2$ appears at 30\%. The same dissociation is universal across architectures: the MLP (156K) peaks in $\lambda$ at 40\% versus $D_2$ at 90\% of EoS; the MLP (269K) at 50\% versus 90\%. In every case the attractor is geometrically most complex not where perturbation sensitivity is greatest, but in the transition zone where chaotic dynamics and basin convergence compete. This cross-architecture dissociation is characteristic of the KAM transition regime~\cite{kam}, where ordered and chaotic regions coexist in fractal interleaving.
...
MLP/CIFAR & 156K & C-10 & $4.61(10)$ & 90 & 10\\
MLP/CIFAR & 269K & C-10 & $4.37(5)$ & 90 & 10\\
...
The findings are consistent with the Newhouse-Ruelle-Takens route to chaos~\cite{ruelle1971,newhouse1978}. Complex data creates a loss landscape with ridges, saddle points, and near-equivalent solutions that provide independent axes of oscillation. When enough of these modes couple through the training dynamics, the flow produces a strange attractor. The label-noise sweep (Fig.~\ref{fig:noise}) reveals that data complexity acts as a continuous control parameter, but its effect is regime-dependent: at moderate learning rates, structured labels create the oscillatory modes that the attractor fills, and degrading them smoothly reduces $D_2$; near the Edge of Stability, even random labels sustain high-dimensional chaos because the rougher loss surface provides its own competing modes. The bounded chaos window is consistent with the KAM picture~\cite{kam}: some invariant structures survive perturbation while others are destroyed. The universality of the $\lambda$--$D_2$ dissociation across architectures---each showing peak geometric complexity offset from peak sensitivity---strengthens this identification: the transition zone where chaos and order coexist produces the richest attractor geometry regardless of the network that mediates the dynamics. The CNN's hierarchical processing creates efficient coupling pathways, lowering the learning rate at which the transition occurs. The MLP accesses the same modes---provided by the data---but requires stronger driving to couple them. On structureless data, no architecture produces multi-dimensional dynamics because the modes do not exist.
