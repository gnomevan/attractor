...
def compute_persistence(points, max_dim=2, max_points=400, subsample_seed=42):
    """
    ...
    if n > max_points:
        idx = np.random.RandomState(subsample_seed).choice(n, max_points, replace=False)
        points = points[idx]
        n = max_points
...
def optimal_delay(x, max_lag=500):
    """
    Optimal delay for Takens embedding.
    Primary: first zero crossing of autocorrelation (standard prescription).
    Fallback: first local minimum if no zero crossing found.
    """
    n = len(x)
    x_c = x - x.mean()
...
def run_tda_experiment(n_seeds=3, n_steps=5000, n_samples=2000, quick=False):
    ...
    if not HAS_RIPSER:
        print(f"  *** WARNING: from-scratch H₁ is a heuristic approximation. ***")
        print(f"  *** Install ripser for reliable results: pip install ripser ***")
    print("=" * 60)
    ...
    for drive_root in ["/content/drive/MyDrive", "/teamspace/studios/this_studio"]:
        ...
        if os.path.isdir(drive_root):
            ...
            drive_path = os.path.join(drive_dir, "tda_cifar10.json")
