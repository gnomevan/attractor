...
- 14,110 parameters (fc1: 11,050, fc2: 2,550, fc3: 510)
- Follows Cohen et al. (2021) setup

**Data:** Synthetic, deterministic (seed 42)
- 2,000 samples, 10 classes
- 200 random features (Gaussian clusters around 10 class centers) + 20 quadratic features (squares of first 20 random features)
...
- λ_max ≈ 7.42 (via power iteration after 1000 warmup steps at lr=0.01)
...
### Method
- Initialize network with seed s, create perturbed copy: weights shifted by ε along a unit-norm random direction in parameter space (direction seeded at s + 999999 for reproducibility)
- Train both identically with full-batch GD
...
### Connection to computational evidence experiments

The companion paper "Computational Evidence for Nested Toroidal Structure" (Experiment 5) tested whether a small 83-parameter MLP showed toroidal training dynamics, and found a negative result — correlation dimension stayed near 1.0 across all learning rates. The chaos onset experiments use a 14,110-parameter network and *do* find genuine chaos, including the isotropic KAM-torus-destruction signature from Experiment 3. This is consistent with a scaling story: toroidal and chaotic dynamics in gradient descent require sufficient network complexity to manifest. The 83-parameter network has too few degrees of freedom for multi-directional instability; the 14K-parameter network has enough. This suggests a minimum model complexity threshold for the phenomenon, which the architecture generalization experiments (Phase 2) should characterize.

---

## 6b. Sensitivity Analysis Results (New)

### The ε = 10⁻⁸ measurement was in the noise floor

The perturbation sensitivity sweep (7 ε values × 5 LRs × 3 seeds) revealed that the original perturbation magnitude (ε = 10⁻⁸) was too small to produce reliable function-space Lyapunov exponents.

**Evidence:** At ε = 10⁻¹² and 10⁻¹⁰, all exponents are exactly zero — perturbation below numerical precision. At ε = 10⁻⁸, all five learning rates return approximately the same small positive value (~+0.00003 to +0.00005). This uniformity across learning rates that should produce very different dynamics is a diagnostic of numerical artifact, not genuine sensitive dependence.

**The converged regime:** At ε ≥ 10⁻⁶, the Lyapunov exponents plateau and stop changing with ε. The plateaued values show a qualitatively different — and more interesting — picture:

| η | λ (plateau, ε = 10⁻⁵) | Interpretation |
|---|---|---|
| 0.005 | −0.00004 | Stable, slightly contracting |
| 0.010 | +0.00007 | Marginally positive |
| 0.020 | +0.0002 | Peak chaos |
| 0.040 | +0.0001 | Positive but weaker |
| 0.080 | −0.0006 | Strongly contracting |

### Chaos is non-monotonic: the chaos window

The most important finding is the **non-monotonicity**. Chaos does not increase monotonically with learning rate. It peaks in a window around η ≈ 0.02 and then the Lyapunov exponent goes strongly negative at higher learning rates. This connects directly to Experiment 2's finding that higher learning rates produce more tightly converged functions across seeds.

### Torus interpretation: tube-swelling vs basin-convergence

The non-monotonic Lyapunov curve maps onto a competition between two forces in the torus geometry of the training dynamics:

**Torus destruction** (swells the tube): As learning rate increases, oscillations around the convergence path grow. The torus surface roughens, breaks, and the dynamics become chaotic in parameter space. This increases function-space divergence (positive λ).

**Basin convergence** (pulls the center inward): The loss landscape has a single deep basin (Experiment 2). Higher learning rates cause trajectories to explore the basin more aggressively, sampling it more thoroughly and converging to the functional minimum faster. This decreases function-space divergence (negative λ).

At medium η (~0.02), torus destruction wins → positive λ (chaos window).
At high η (~0.08), basin convergence wins → negative λ (ergodic convergence).

The donut breaks, but the basin swallows the debris. The strange attractor that replaces the torus is confined to the basin, and from outside the basin it looks like convergence.

### Methodological implications

All subsequent experiments use ε = 10⁻⁵ (safely in the plateau). The transition zone sweep range is expanded to [0.005, 0.15] to capture the full arc: onset, chaos window, and reconvergence. The η_c estimate from the original 3-seed experiments (0.018 ± 0.012) was based on noise-floor measurements and should be superseded by 20-seed results at proper ε.
...
- Transition zone with 20 seeds at ε = 10⁻⁵ (tighten η_c, map chaos window shape)
- Broad sweep with 20 seeds
- ~~Perturbation sensitivity analysis~~ → **COMPLETED** (see Section 6b)
...
- Why does η_c sit at ~6.6% of 2/λ_max? Is there a theoretical prediction for this ratio? (Note: η_c estimate pending revision with proper ε)
...
- Why is chaos non-monotonic? At what η does λ peak, and does the peak η depend on architecture? Is the reconvergence at high η universal for gradient descent with single-basin landscapes?
- Does the chaos window width (the range of η with positive λ) change with network depth/width? Ruelle-Takens predicts more frequencies → easier chaos, which should widen the window.
...
**The sensitivity analysis adds a further novel finding:** chaos is non-monotonic with learning rate, peaking in a window and giving way to basin-dominated convergence at higher η. This "chaos window" picture — torus destruction vs basin convergence — is a new result with clear geometric interpretation. Combined with the non-monotonicity, the paper's contribution shifts from "chaos begins earlier than expected" to "gradient descent passes through a chaos window, bounded on both sides by convergence for different reasons." This is a stronger and more distinctive claim.
